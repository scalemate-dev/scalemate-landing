# Run seo-analysis agent for slug: seo-analysis-2026-04-30-v2
...
1. Commit artifacts: `seo-system-v1/output/topics/seo-analysis-2026-04-30-v2/` + `seo-system-v1/workflow/{pipeline,scorecard}.md`
2. Push current branch
3. PR — **only create if missing**, never duplicate:
   ```bash
   if [ -z "$(gh pr list --head "seo/seo-analysis-2026-04-30-v2" --json number --jq '.[0].number')" ]; then
     gh pr create --title "SEO: seo-analysis-2026-04-30-v2" --body "Pipeline branch for topic seo-analysis-2026-04-30-v2. Each agent (discovery → content-creator → qa) pushes commits here. Merge only after QA pass + deploy approval."
   fi
   ```
