...
| facebook-ads-automation-listicle-rework | 250 (cluster) | 6–14 | — | MOFU | TBD | **NEW 2026-05-07 — S5' replacement for killed `facebook-ads-automation-tools-comparison` (in §9 Rejected).** Rework of existing `/blog/facebook-ads-automation` (in §8 Published) → listicle format ("12 Best Facebook Ads Automation Tools 2026"). Same URL preserves equity. Cluster: `facebook ads automation` (150 vol KD 6) + `…tools` (50 KD 14) + `…software` (40 KD 12). SERP analysis 2026-05-05 confirms listicle format wins (adstellar #4 24 traffic, itsfundoingmarketing #6 22 traffic) over comparison guide (get-ryze.ai DR 70 = 3 traffic). Existing tools section already has Birch, Madgicx, Scalemate, Advantage+ embedded — expand to 12 tools, drop guide framing, add comparison table at top. After rework: re-submit GSC URL inspection + LinkedIn share for crawl signal. Internal links from `/features/automation-rules` + `/use-cases/ad-campaign-automation-rules` for indexation push. **Discovery agent task:** validate listicle structure (which 12 tools, ordering rationale, table columns, peer-set check), produce brief.md before write. Full S5' notes: `action-lists/2026-05-04.md` S5'. T+2wk milestone (2026-05-19) post-deploy: if still not indexed, escalate to URL change strategy. |
