...
Usage:
    ...
    python3 seo-system/scripts/detect-metadata-changes.py --days 60
    python3 seo-system/scripts/detect-metadata-changes.py --no-gsc

Requires (only without --no-gsc):
    GSC_SITE_URL env var, .gsc-token.json — see .claude/skills/seo-ops/gsc_client.py
    Optionally GSC_TOKEN_FILE / GOOGLE_CREDENTIALS_FILE if the defaults
    in gsc_client.py don't resolve to your local files.
"""
...
def main():
    ...
    for url in sorted(summarized):
        ...
        next_check = d + timedelta(days=COOLDOWN_DAYS)

        print(
            f"| `{url}` metadata | {deploy_date} | "
            f"{fmt_metrics(baseline)} | {fmt_metrics(current)} | "
            f"{next_check} (T+2wk) | {change_summary} | monitoring |"
        )
