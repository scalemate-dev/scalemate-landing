...
8. **Audience-Language Filter (§6 Правило 8)** — grep по reader-side жаргон:
   ```
   ...
   ```
   Кожен hit = critical issue. Плюс sanity scan: для кожного абзацу спитай "буде це цікаво primary reader role статті (media buyer / AdOps / performance marketing lead / CMO / CEO / founder залежно від ICP)"? Якщо ні — flag для content-creator як audience-mismatch.
