...
export const GOAL_LABELS = Object.fromEntries(
  RULE_GOALS.map((g) => [g.id, g.label]),
)
export const LEVEL_LABELS = Object.fromEntries(
  RULE_LEVELS.map((l) => [l.id, l.label]),
)
export const OBJECTIVE_LABELS = Object.fromEntries(
  RULE_OBJECTIVES.map((o) => [o.id, o.label]),
)
...
const ALL_RULES = [
  ...
  },
  {
    ...
    whenToUse:
      "A combined kill-and-scale rule over a 3-day window — pauses ad sets spending 3× CPA with cost-per-purchase past 2× CPA (clearly underperforming), and aggressively scales ad sets that have spent at least 1.8× CPA with cost-per-purchase under 0.9× CPA (clearly profitable). The aggressive 50% bump is designed to push early-winner ad sets to a higher budget tier as fast as auction-signal allows — once an ad set shows strong unit economics on 1.8× CPA spent, the team wants to capture that performance before competitors catch the audience overlap. Performance teams use this for early-winner escalation without manual triage.",
    nativeLimitation:
      "Meta evaluates the pause and the scale tasks independently — an ad set that crossed the 1.8× CPA scale floor can also hit the 3× CPA pause threshold the same evaluation hour and end up paused before the budget bump takes effect. Native rules can't express 'scale only if pause didn't fire' inside a single rule. The 3-day window also blends spend across all three days, so a single late-attribution purchase can flip the cost-per-purchase reading just before evaluation. There's no automatic rollback if the boosted ad set degrades the next day — pair with a separate rollback rule.",
  },
  {
    id: "rule-104",
  ...
  },
  {
    ...
    nativeLimitation:
      "Meta evaluates cost per checkout on attribution-window data, so the rule can fire on numbers that haven't fully settled. There's also no way to pair this with a creative-fatigue check in one rule — if you want to skip ads in the learning phase, that requires a second rule and manual reconciliation.",
  },
  {
    id: "rule-24",
    objectives: ["sales"],
    ...
    whenToUse:
      "Conservative campaign-level autoscaler — small 10% bumps when CPA is comfortably under breakeven, 20% cuts when 3 days of fair-test spend (1.8× CPA) come back at 1.3× CPA or worse. Useful for evergreen campaigns where the team wants steady growth without triggering Meta's learning reset. The asymmetry — slow up, fast down — protects budget from drift.",
    nativeLimitation:
      "Meta won't catch the case where the same campaign hits both conditions on different ad sets — the scale fires on the campaign level even if 90% of the spend is wasted on losers underneath. The rule reads aggregate, so audit ad-set splits separately.",
  },
  {
    id: "rule-25",
    objectives: ["sales"],
    ...
    whenToUse:
      "An outlier ceiling, not your primary pause rule. Meta attribution lags 24-48h on most placements and stretches further on iOS, so pausing fast on today's CPA often kills ad sets that recover tomorrow. 2.8× breakeven is set deliberately past the lag noise band — anything above is broken, not late. Pair with a zero-purchase rule and a 3-7 day pause rule for systematic pruning.",
    nativeLimitation:
      "CPP is undefined when purchases = 0, so this rule never fires on pure budget burners — you need a separate spend > X AND purchases < 1 rule for that. If false pauses appear on top-funnel ad sets, drop the cadence to every 5-6 hours to give attribution time to land.",
  },
  {
    id: "rule-26",
    objectives: ["sales"],
    ...
    whenToUse:
      "A creative-quality canary — CPC north of $5.50 on a $10+ spend usually flags a hook or thumbnail problem before the conversion data even arrives. Mobile apps studios use this on broad-audience tests where the ad's job is to earn cheap clicks first. If the team's average CPC sits below $2, drop the threshold proportionally.",
    nativeLimitation:
      "Link CPC ignores CTR drift — an ad with a $5.50 CPC and 4% CTR is dying on hook quality, while one with a $5.50 CPC and 0.6% CTR is dying on relevance. Native rules can't read both metrics in the same condition; if you need that nuance, split into two rules.",
  },
  {
    id: "rule-34",
    objectives: ["sales"],
    ...
    whenToUse:
      "Spend-based hard stop for ad sets that haven't produced a single purchase by mid-day — at 2.5× CPA spent and zero conversions, the auction has had enough signal. Performance teams running multi-million monthly spend use this as the simplest, most defensible kill rule because it avoids ROAS lag entirely. Only use on ad sets past the learning phase.",
    nativeLimitation:
      "Meta evaluates 'today' in the ad account's reporting timezone, not the team's. If the account is set to PST and the team works UTC+2, the rule resets at a different moment than expected — easy to miss when reviewing why something paused at midnight.",
  },
  {
    id: "rule-37",
    objectives: ["sales"],
    ...
    whenToUse:
      "ROAS guardrail for evergreen BAU campaigns, not test runs. For e-com and mobile apps, purchase count lies — one high-value sale can pay the whole campaign while ten cheap ones return 20%. After 20× CPA spent (enough volume to read true ROAS), pause anything below 0.5× target ROAS, trim 30% budget on borderline cases below 0.75× target.",
    nativeLimitation:
      "ROAS lag matches CPA lag but hurts more — value-weighted conversions arrive unevenly, and one large delayed sale can swing 3-day ROAS by 20%+. For high-AOV categories (luxury, B2B SaaS, premium apps), widen the window to last 7d to smooth out outliers.",
  },
  {
    id: "rule-38",
    objectives: ["sales"],
  ...
  },
  {
    ...
    whenToUse:
      "Creative-pruning play for ad sets with 3+ active creatives. When one ad hogs the budget but doesn't convert over 3 days, pausing it forces Meta's auction to redistribute spend to siblings that may perform better. Not a permanent kill — the goal is to free up testing room for competing creatives, not eliminate the ad forever. Skip this rule on single-ad sets — it'll just shut the set down.",
    nativeLimitation:
      "Meta doesn't redistribute the paused ad's budget proportionally to specific siblings — the ad set re-enters the learning auction and reallocates based on its current model, which may favor any ad, not the next-best historical performer. For deterministic shifts, use ad-set level rules instead.",
  },
  {
    id: "rule-41",
    ...
    tasks: [
      {
        ...
        action:
          "Increase campaign budget by 200% and duplicate campaign with minimum budget",
      },
      {
        timeframe: "yesterday",
        conditions: [
  ...
  },
  {
    ...
    whenToUse:
      "Conservative scaler for ad sets that have proven themselves over 3 days — at least 5 purchases at CPA below your breakeven target earns a 20% budget bump. The 5-purchase floor is what makes the signal trustworthy; one-conversion days don't trigger the rule. Best for evergreen campaigns where the team wants smooth ramping rather than spiky scale.",
    nativeLimitation:
      "Native rules can't enforce a maximum budget ceiling on the scale action — at daily +20% the ad set doubles in 4 days of consecutive winning performance. Set a manual review cadence or chain to a max-budget rollback rule before the bump compounds beyond what the team can support.",
  },
  {
    id: "rule-45",
    objectives: ["sales"],
  ...
  },
  {
    ...
    whenToUse:
      "Modest scaler for campaigns showing real signal — at least 2 purchases on 4× breakeven CPA spent in a day earns a 10% budget bump. Conservative on purpose; the rule fires every 3 hours, so a campaign with sustained performance can compound to a 30-40% bump by end of day without manual intervention. Use on stable campaigns past the learning phase.",
    nativeLimitation:
      "The rule has no upper bound — if conditions hold for 8 hours, Meta will keep firing the 10% bump every 3 hours. Compounding is the feature, but it's also the risk. Pair with a daily budget cap rule that fires when total daily spend exceeds the team's cap.",
  },
  {
    id: "rule-51",
    draft: true,
    ...
    whenToUse:
      "Two-sided campaign rule with sharp thresholds — fewer than 3 purchases at 4× CPA spend is a kill, more than 5 purchases at 5× CPA spend is a 30% scale. Aggressive for performance teams running hundreds of campaigns where the cost of slow reaction outweighs the cost of occasional false pauses. Keep on tight cadence to capture the winner side fast.",
    nativeLimitation:
      "Both tasks evaluate the same hour — a campaign with exactly 4 purchases at $230 spend triggers neither, sitting in the dead zone. Meta can't apply continuous logic in one rule, so edge cases between thresholds require either a third 'middle band' rule or manual review of mid-tier campaigns.",
  },
  {
    id: "rule-52",
    draft: true,
    ...
    nativeLimitation:
      "When checkouts initiated = 0, the cost-per-checkout metric is technically undefined or returned as the spend amount itself depending on Meta's evaluation path. The rule still fires correctly, but downstream reporting can show inconsistent values. Verify behaviour on a low-spend test before scaling to high-budget ads.",
  },
  {
    id: "rule-54",
    draft: true,
    ...
    whenToUse:
      "Lifetime spend-based kill rule — once an ad has burned 1.2× CPA across its full life with zero checkout-inits, it goes off. Used on accounts where the team launches and rotates creative aggressively and wants quick lifetime cleanup. The 'maximum' window means once paused, the rule's job is done — the data point becomes permanent.",
    nativeLimitation:
      "The 'maximum' (lifetime) timeframe ignores recent improvement — an ad that didn't drive checkouts in week one but is now performing fine still carries the original deficit. Once near the lifetime spend floor, the rule will fire on its old data. For ads that recovered, manual reactivation is the only fix.",
  },
  {
    id: "rule-55",
    draft: true,
    ...
    whenToUse:
      "Specifically catches the leaky-funnel ad — checkouts are initiating but never converting, even at 2× CPA lifetime spend. This often indicates the audience is right but the post-click experience or pricing isn't matching, or the ad is attracting browsers, not buyers. DTC e-commerce teams use this to surface ads that look healthy on top-of-funnel metrics but are quietly draining budget.",
    nativeLimitation:
      "Three-condition AND clauses are the limit of native rule logic — there's no way to add a fourth condition like 'and link CTR > 1.5%' to confirm the audience-quality angle. To diagnose the funnel break, the team has to pause first and investigate manually.",
  },
  {
    id: "rule-56",
    draft: true,
  ...
  },
  {
    ...
    whenToUse:
      "High-AOV CPA cap — once a single ad pushes past 3.2× breakeven CPA with 6× CPA spent in a day, it's in clear loss territory. The 6× spend floor ensures the rule fires on real signal, not on early auction noise — at $50 CPA that's $300, at $150 CPA it's $900. Performance teams running large daily budgets per ad use this as the per-ad guardrail.",
    nativeLimitation:
      "Same-day evaluation with a $300 spend floor means the rule rarely fires before mid-afternoon — on accounts with strict daily caps, the worst damage may already be done by then. Pair with a tighter ad-set-level rule that catches the pattern earlier in the day.",
  },
  {
    id: "rule-61",
    draft: true,
    ...
    whenToUse:
      "Mid-cycle kill rule for ads that have spent 4× CPA over 3 days without producing a single checkout. The 3-day window absorbs daily volatility, and zero checkouts at this spend is a clean signal that the ad isn't reaching purchase intent at all. Alert via Slack on the same threshold if the team prefers human review on borderline ads.",
    nativeLimitation:
      "Checkout-init events depend on pixel firing — if the conversion API is intermittent or the pixel deduplication is misaligned, the count can read 0 even when 1-2 events occurred. Verify pixel health before relying on this rule for ads with significant spend.",
  },
  {
    id: "rule-62",
    draft: true,
    ...
    whenToUse:
      "Three-step ad-set kill chain that covers lifetime spend, today's spike, and registration funnel separately. Used on funnels where registration is the early signal and purchase is the lagging signal — if either fails, the ad set comes down. Mobile apps studios with hybrid registration-then-purchase funnels use this to catch breakage at either stage.",
    nativeLimitation:
      "Three independent kill conditions on the same entity means one bad day on the today-window task can pause an ad set that's healthy on lifetime metrics. There's no way to require all three to be triggered simultaneously inside a single rule, so prioritisation between the conditions is set by Meta's internal scheduler, not the team's intent.",
  },
  {
    id: "rule-63",
    draft: true,
    ...
    whenToUse:
      "Catches two failure modes — expensive checkouts with no purchase under 2× CPA lifetime spend, and 1× CPA spend with zero registration signal. The first task is for funnels where checkouts predict purchase reliably; the second covers accounts where registration is the early gate. Useful on lead-gen-heavy funnels.",
    nativeLimitation:
      "The two tasks fire independently — an ad meeting the registration condition (no signups at the spend floor) but not the checkout condition still pauses, even if other behavioural signals look fine. Native rules can't enforce 'pause only if both fail' logic; that requires a chained automation outside Meta.",
  },
  {
    id: "rule-65",
    draft: true,
  ...
  },
  {
    ...
    whenToUse:
      "Per-ad-set burn-rate brake — 15× breakeven CPA over 3 days = roughly 5× CPA per day, which is heavy budget concentration on a single ad set. The 5-purchase floor under-correlates with that spend (one conversion per ~3× CPA, well below breakeven), so the rule trims 25% to bring pacing back in line. Performance teams running tight monthly caps deploy this as the per-ad-set guardrail alongside campaign-level stop-loss rules.",
    nativeLimitation:
      "Native rules re-fire every 5 hours on the same condition without a budget floor — repeated trims compound, leaving the ad set at ~30% of its original budget after 3-4 firings without anyone noticing. Pair with a manual review cadence or a reactivation rule on confirmed conversion recovery.",
  },
  {
    id: "rule-100",
    draft: true,
    ...
    whenToUse:
      "Phase 1 of mobile UA creative testing — kills underperforming creatives early. Mobile apps studios running app install campaigns deploy this on every new ad in test rotation. Once an ad has burned 10× target CPI without converting installs at the right cost, the test is over. Hourly cadence at the ad level catches the pattern before the auction wastes a day's testing budget.",
    nativeLimitation:
      "Meta's native rules can't multiply one user-defined metric by another — there's no way to express 'spend > 10× CPI target' as a dynamic threshold. The spend floor has to be hardcoded as a fixed dollar amount, which means recalculating it manually whenever the team's CPI target changes.",
  },
  {
    id: "rule-101",
    draft: true,
    ...
    source: "playbook",
    title:
      "Pause testing ad when CPM exceeds your CPM benchmark after 10K impressions",
    goal: "stop-loss",
    testingPhase: 1,
    level: "ad",
    platform: "meta",
    ...
    whenToUse:
      "After 10K impressions you have a fair test sample for CPM-based delivery cost. If CPM is 25% above the team's benchmark, the auction is paying too much for impressions — either the creative isn't winning attention or the audience is saturating. Mobile apps studios use this as the second Phase 1 kill alongside the CPI rule, catching ads that burn impressions before they ever convert.",
    nativeLimitation:
      "Meta's CPM benchmark for an account is a moving target — relative comparisons (1.25× of a rolling baseline) aren't supported in native rules. The threshold has to be a static number, recalculated as auction prices shift seasonally and as the account's audience overlap changes.",
  },
  {
    id: "rule-102",
    draft: true,
    ...
    whenToUse:
      "After spending 2× breakeven CPA without securing the second purchase, the ad has had a fair test on a Sales/AEO/ROAS campaign. Use this on direct-response setups where the team accepts losing one CPA per failed test as the cost of creative volume — Phase 2 is where surviving Phase 1 creatives prove they can convert past a single fluke.",
    nativeLimitation:
      "Meta natively can't multiply spend by your breakeven CPA — you set the floor as a fixed dollar amount and update it whenever your unit economics change. There's no way to express '2× CPA' as a live threshold inside the rule engine.",
  },
  {
    id: "rule-103",
    draft: true,
    ...
    whenToUse:
      "Phase 3 is the deep test — by 10× CPA spent over 3 days, the creative has had every chance to prove itself on a Sales/AEO/ROAS objective. ROAS at 80% of target means the creative is structurally below breakeven; further spend won't fix it. Use as the final gate before a creative graduates to evergreen rotation or gets retired.",
    nativeLimitation:
      "Native rules can't compound a spend-multiple condition with a ROAS condition relative to the team's target. Both have to be hardcoded as absolute thresholds and updated when the team's unit economics or ROAS target shifts.",
  },
  {
    id: "rule-105",
    objectives: ["sales", "app-promotion"],
  ...
  },
  {
    ...
    tasks: [
      {
        ...
        action:
          "Send Slack notification to creative team with hook-rate winners",
      },
    ],
    whenToUse:
    ...
    nativeLimitation:
      "Meta natively exposes 3-second video plays and impressions, so the hook rate calculation is feasible inside a native rule. What native rules can't do: route the alert into a specific Slack channel, attach the actual creative thumbnail to the notification, or group multiple winners into a single digest. Scalemate's tracking layer pipes the alert into a dedicated creative-team channel with the ad's thumbnail and copy so designers can act immediately.",
  },
  {
    id: "rule-107",
    objectives: ["sales", "app-promotion"],
