...
function renderMarkdown(content) {
  ...
  renderer.link = ({ href, title, tokens }) => {
    ...
    // Parse custom attributes like {rel="nofollow" target="_blank"}
    const attrMatch = href.match(/^(.+?)\{(.+?)\}$/)
    let cleanHref = href
    let extraAttrs = ""
    if (attrMatch) {
      cleanHref = attrMatch[1]
      extraAttrs = " " + attrMatch[2]
    }

    const isExternal = cleanHref.startsWith("http")
    const target = isExternal ? ' target="_blank" rel="noopener noreferrer"' : ""

    return `<a href="${cleanHref}"${target}${extraAttrs}${title ? ` title="${title}"` : ""}>${inner}</a>`
  }
  ...
  return marked.parse(content, { renderer, gfm: true })
}
